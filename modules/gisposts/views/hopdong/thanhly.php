...
$this->params['breadcrumbs'][] = 'Hợp đồng thanh lý';
...
<div class="row">
        ...
        <div class="portlet box blue-steel">
            ...
            <div class="portlet-body">
                <table class="table table-scrollable-borderless table-responsive">
                    <tr class="bg-success">
                        <th><a href="<?= Yii::$app->urlManager->createUrl(['quan-ly/hopdong/conhan']) ?>">Còn
                                hạn</a></th>
                        <td align="right"><?= $model['total']['conhan'] ?></td>
                    </tr>
                    <tr class="bg-warning">
                        <th><a href="<?= Yii::$app->urlManager->createUrl(['quan-ly/hopdong/saphethan']) ?>">Sắp
                                hết hạn</a></th>
                        <td align="right"><?= $model['total']['saphethan'] ?></td>
                    </tr>
                    <tr class="bg-danger">
                        <th><a href="<?= Yii::$app->urlManager->createUrl(['quan-ly/hopdong/hethan']) ?>">Hết
                                hạn</a></th>
                        <td align="right"><?= $model['total']['hethan'] ?></td>
                    </tr>
                    <tr class="bg-info">
                        <th><a href="<?= Yii::$app->urlManager->createUrl(['quan-ly/hopdong/chuanhap']) ?>"
                            >Chưa nhập</a></th>
                        <td align="right"><?= $model['total']['chuanhap'] ?></td>
                    </tr>
                    <tr class="bg-default">
                        <th><a href="<?= Yii::$app->urlManager->createUrl(['quan-ly/hopdong/khongco']) ?>">Không
                                có</a></th>
    ...
    <?php ActiveForm::begin([
        'id' => 'export-hopdong',
        'action' => Yii::$app->urlManager->createUrl('hopdong/export'),
    ]) ?>
    <div class="col-lg-12">
        ...
        <?= GridView::widget([
            ...
            'rowOptions' => function ($model) {
                ...
                } elseif (strtotime($model->ngay_het_han) < strtotime(date('d-m-Y'))) {
                    return [
                        'class' => 'bg-danger'
                    ];
                } elseif (strtotime($model->ngay_het_han . '- 1 month') < strtotime(date('d-m-Y'))) {
                    return [
                        'class' => 'bg-warning'
                    ];
                } else {
            ...
            'toolbar' => [
                ['content' =>
                    ...
                    "<button type='submit' class='btn btn-default'>Xuất file</button>"
    ...
    <input type="hidden" name="SearchHopdong[so_hop_dong]" value="<?= $model['search']->so_hop_dong ?>">
    <input type="hidden" name="SearchHopdong[nguoi_thue]" value="<?= $model['search']->nguoi_thue ?>">
    <input type="hidden" name="SearchHopdong[so_nha]" value="<?= $model['search']->so_nha ?>">
    <input type="hidden" name="SearchHopdong[ten_duong]" value="<?= $model['search']->ten_duong ?>">
    <input type="hidden" name="SearchHopdong[ma_phuong]" value="<?= $model['search']->ma_phuong ?>">
    <input type="hidden" name="SearchHopdong[thoi_han_thue]" value="<?= $model['search']->thoi_han_thue ?>">
    <input type="hidden" name="SearchHopdong[loainha]" value="<?= $model['search']->loainha ?>">
    <?php ActiveForm::end() ?>
    <div style="clear: both"></div>
</div>