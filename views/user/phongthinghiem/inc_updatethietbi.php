...
<script>
    $('#userupdate-thietbi').on('beforeSubmit', function(e) {
        var form = $(this);
        var formData = form.serialize();
        $.ajax({
            url: '<?=Yii::$app->urlManager->createUrl('user/userupdatethietbi?id=').$_GET['id']?>',
            type: 'POST',
            data: formData,
            success: function (data) {
                reloadTable('#tab_userthietbi','<?= Yii::$app->urlManager->createUrl('user/userlistthietbi?id=').$model['thietbi']->id_pdk?>');
                $('#ajaxModalTb1').remove();
